...
;(async () => {
  ...
  const r31 = await p.evaluate(() => { const css = [...document.styleSheets].flatMap(sh => { try { return [...sh.cssRules] } catch { return [] } }); const kf = css.find(r => r.name === 'card-swirl'); const last = kf ? [...kf.cssRules].pop().style.transform : ''; const note = document.querySelector('#tier [data-mitra-note] p'); return { hook: getComputedStyle(document.getElementById('hook')).backgroundColor, hero: getComputedStyle(document.getElementById('hero')).backgroundColor, swirl: last, banner: !!document.querySelector('#benefit [data-steps] .rounded-xl.bg-white'), kicker: /Cara kerjanya/i.test(document.querySelector('#benefit [data-steps]').innerText), noteFam: note ? getComputedStyle(note).fontFamily : '', kl: getComputedStyle(document.getElementById('klasemen')).backgroundColor, klSpark: document.querySelectorAll('#klasemen svg.lucide-sparkles').length, jost: [...document.fonts].some(f => /Jost/.test(f.family)) } })
  ...
  ok(/Futura|Jost/.test(r31.noteFam) && r31.jost, `Mitra note in the Futura stack, Jost face registered (${r31.noteFam.slice(0, 30)})`)
