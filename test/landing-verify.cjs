...
;(async () => {
  ...
  const mc = await p.evaluate(() => { const c = document.querySelector('#benefit [data-member-card]'); const t = getComputedStyle(c).transform; return { swoosh: !!c.querySelector('svg.swoosh path'), chip: !!c.querySelector('.bg-gold'), id: /RMC 2026 0001 0003/.test(c.textContent), persp: /matrix3d/.test(t) } })
  ok(mc.swoosh && mc.chip && mc.id && mc.persp, `member card: green swoosh, chip, member id, rests in perspective (${JSON.stringify(mc)})`)
