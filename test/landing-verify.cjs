...
;(async () => {
  ...
  const heads = await p.evaluate(() => [...document.querySelectorAll('#tier .tier-head')].map(h => getComputedStyle(h).backgroundColor))
  ok(heads.length === 6 && new Set(heads).size === 6, `tier cards carry a solid coloured header each (${heads.length} distinct)`)
