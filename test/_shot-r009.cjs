...
;(async () => {
  ...
  await p.locator('#benefit ul > li > div').nth(1).hover(); await p.waitForTimeout(500)
  await p.locator('#benefit').screenshot({ path: 'test/_shots/r010-benefit-hover.png' })
  await p.locator('[data-rmc-card]').hover(); await p.waitForTimeout(600)
  await p.locator('#cek-poin').screenshot({ path: 'test/_shots/r010-cek-poin-hover.png' })
