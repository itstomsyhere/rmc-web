      ...
      <SettingsCard title="Pelanggan CRM (mock)" desc="Master pelanggan Resique. Nomor HP terisi otomatis setelah pendaftaran jalur 2 (RSL + nama cocok) — backfill." bodyClassName="p-0">
        <Table>
          ...
          <TBody>
            {customers.map(c => (
              <TR key={c.id}>
                <TD className="pl-5 font-mono text-[12px]">{c.rsl || <span className="text-ink-4">—</span>}</TD>
  ...
  return (
    <li className="rounded-xl border border-line p-4">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2"><span className="font-mono text-[12px] text-ink-3">{claim.id}</span><Badge variant={claim.score >= 0.9 ? 'ok' : 'warn'}>skor {Math.round(claim.score * 100)}%</Badge></div>
...
function ResolvedRow({ claim }: { claim: Claim }) {
  ...
  return (
    <TR>
      <TD className="font-mono text-[12px]">{claim.id}</TD>
