...
export const Progress = React.forwardRef<React.ElementRef<typeof ProgressPrimitive.Root>, React.ComponentPropsWithoutRef<typeof ProgressPrimitive.Root> & { tone?: 'teal' | 'gold' }>(({ className, value, tone = 'teal', ...props }, ref) => (
  <ProgressPrimitive.Root ref={ref} className={cn('relative h-2 w-full overflow-hidden rounded-full bg-black/10', className)} {...props}>
    <ProgressPrimitive.Indicator className={cn('h-full w-full flex-1 rounded-full transition-transform duration-700 ease-out', tone === 'gold' ? 'gold-gradient' : 'bg-teal')} style={{ transform: `translateX(-${100 - Math.min(100, Math.max(0, value || 0))}%)` }} />
...
export const TR = ({ className, ...p }: React.HTMLAttributes<HTMLTableRowElement>) => <tr className={cn('border-b border-line-2 transition-colors hover:bg-surface-2/70 data-[state=selected]:bg-teal-50', className)} {...p} />
