...
const RadioGroupItem = React.forwardRef<React.ElementRef<typeof RadioGroupPrimitive.Item>, React.ComponentPropsWithoutRef<typeof RadioGroupPrimitive.Item>>(({ className, ...props }, ref) => (
  <RadioGroupPrimitive.Item ref={ref} className={cn('aspect-square h-5 w-5 rounded-full border-2 border-ink-4 text-navy transition-colors data-[state=checked]:border-navy focus:outline-none focus-visible:ring-2 focus-visible:ring-navy/30 disabled:cursor-not-allowed disabled:opacity-50', className)} {...props}>
    <RadioGroupPrimitive.Indicator className="flex items-center justify-center"><span className="h-2.5 w-2.5 rounded-full bg-navy" /></RadioGroupPrimitive.Indicator>
...
export function RadioCard({ value, title, desc, disabled, badge, id }: { value: string; title: string; desc?: string; disabled?: boolean; badge?: React.ReactNode; id?: string }) {
  ...
  return (
    <label htmlFor={rid} className={cn('lift flex cursor-pointer items-start gap-3 rounded-xl border border-line bg-white p-4 active:scale-[.98] has-[[data-state=checked]]:border-navy has-[[data-state=checked]]:bg-navy-50/60 hover:border-ink-4', disabled && 'cursor-not-allowed opacity-55')}>
